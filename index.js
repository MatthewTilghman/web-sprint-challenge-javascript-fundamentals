...
//🚀🚀🚀 ⬇️ 📝 Explanation ⬇️ 📝 🚀🚀🚀: 
//TODO internal is nested within nestedFinction() & is part of myFunction() scope
...
const zooAnimals = [
  ...
  */
  function animalNames(zoo){
    const displayNames = [];
  zoo.forEach(item => displayNames.push(`name: ${item.animal_name}, scientific: ${item.scientific_name}`));
    return displayNames;
  }
  ...
  */
  function lowerCaseNames(zoo){
    ...
    return zoo.map(item => item.animal_name.toLowerCase());
  }
  ...
  */
  function lowPopulationAnimals(zoo){
    const lowPopCount = zoo.filter(item => item.population < 5)
    return lowPopCount;
  }
  ...
  */
  function USApop(zoo){
    const Population = zoo.reduce((accumulator, item) => accumulator + item.population,0);
    return Population;
  }
...
const cuboid = new CuboidMaker ({length: 4, width: 5, height: 5}); //TODO come back.. forgot capital a in surfaceAREA
